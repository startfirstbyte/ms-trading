...
async def _init_pg_schema(pool: "config.asyncpg.Pool") -> None:
    ...
    async with pool.acquire() as conn:
        ...
        # Price channels — persisted parallel channels with a lifecycle:
        #   status='editing'   → channel còn re-fit mỗi nến đóng (1 dòng editing/symbol:res).
        #   status='committed' → giá đã đóng vượt biên + buffer → đóng băng, không sửa nữa.
        # Partial unique index đảm bảo tối đa 1 editing cho mỗi (symbol, resolution).
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS price_channels (
                id           BIGSERIAL PRIMARY KEY,
                symbol       TEXT NOT NULL,
                resolution   TEXT NOT NULL,
                status       TEXT NOT NULL DEFAULT 'editing',  -- 'editing' | 'committed'
...
async def _get_channels(symbol: str, resolution: str, committed_limit: int = 3) -> list[dict]:
    """Channel editing (nếu có) + đúng `committed_limit` committed mới nhất.
