...
## Long-form docs (Obsidian)

Lịch sử/bối cảnh đầy đủ (quá dài cho skill này) sống trong Obsidian vault, `Trading Project/`:
[[ai-signal-lock]] (chi tiết đầy đủ active-signal lock — §"Active-signal lock" ở trên là tóm tắt),
[[ai-cost-tuning]] (structured output, scoped analyze, tuning prompt — bối cảnh AI_BACKEND),
[[ai-auto-killswitch]] (2 cơ chế auto-analysis độc lập, dễ nhầm khi debug chi phí),
[[2026-06-22-h1-timeout]] (incident — vì sao H1 disabled, liên quan §"Điểm yếu #1 ZigZag không
adaptive" ở trên). Query theo `related-skill: technical-analysis` qua `mcp__obsidian__search_query`
nếu cần liệt kê hết.
